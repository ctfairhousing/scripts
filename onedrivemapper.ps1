...
$onedriveIconPath = "C:\scripts\onedrivemapper.ico" #if this file exists, and you've set addShellLink to True, it will be used as icon for the shortcut
...
if((checkRegistryKeyValue -basePath "HKLM:\SYSTEM\CurrentControlSet\Services\WebClient\Parameters\" -entryName "SupportLocking") -ne 0){
    log -text "ERROR: WebDav File Locking `upport is enabled, this could cause files to become locked in your OneDrive or Sharepoint site" -fout 
} 
...
Set-Itemproperty -path 'HKLM:\SYSTEM\CurrentControlSet\Services\WebClient\Parameters\' -Name SupportLocking -Value 0 
